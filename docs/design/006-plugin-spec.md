...
> **v0.2 变更(2026-09-11)**:助手(插件)不再做多版本并存管理。`plugins` 表按 `name` 全局唯一,
> 同名插件重新部署 = **原地覆盖**(保留行 UUID 与历史会话,替换清单/代码与私有注册表资源)。
> `version` 字段仍要求 semver,但仅作为"最近部署版本"的展示标签,不再参与唯一性与解析。
> 平台**公共注册表资源**(builtin/shared 的 skill/tool/kb)仍保留多版本与 `^`/`~` 约束
> ——这是平台第一特色(细粒度能力复用)的版本基础,本次不简化。
...
## 2. plugin.yaml 清单
```yaml
name: prd-review-assistant       # 全局唯一;同名重部署原地覆盖(见 §7)
version: 0.1.0                   # semver,仅"最近部署版本"标签,不参与唯一性
author: <developer>
...
4. 平台:再次校验清单 -> 解析 `depends_on`(注册表校验版本)-> 校验命名空间/不覆盖内置 -> 登记
5. 返回 `{plugin_id, status}`

### 7.1 同名覆盖语义(v0.2)
- `plugins.name` **全局唯一**:首次部署插入新行;同名再次部署 = 原地 upsert。
- 覆盖时:
  - **保留行 UUID(`plugin_id`)与首次部署者 owner_id**,故历史会话(`sessions.plugin_id`)不悬挂,自动用上新代码/新清单;
  - 替换 `manifest` / `version` 标签 / `deployed_at`,状态重置为 `active`;
  - 先删除该插件名下(`skill_tools.owner_id = plugin.name`,source=private)的**全部**私有注册表行(含旧版本),再登记本次清单的资源;
  - 清理并重建 `~/.agentplatform/installed_plugins/<name>/` 代码目录。
- 公共资源(builtin/shared)与其他插件的私有资源不受影响;
  自有资源与公共资源撞 `id+version` 仍按 T11.10 规则拒绝。
- 平台为可信开发者内部部署场景(001 §5),同名覆盖不做属主比对;后续多租户化时再加权限。
...
| 插件生命周期 | 启停 / 卸载;**不保留历史版本**,同名重部署原地覆盖(v0.2,见 §7.1) |
