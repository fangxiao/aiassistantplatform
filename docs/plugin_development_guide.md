...
## ⚠️ 4.1 多步工具编排的模型遵循度(重要)

Agent 对**多步工具编排**的执行遵循度是概率性的:同一份含"必须调用某工具"清单的 skill 提示词,在用户显式提及工具名时通常 100% 触发;但在裸业务提示词下,模型可能跳过清单中的部分步骤(实测中"生成配图"类步骤最易被跳过,而"落盘预览"类步骤几乎总是执行)。

**插件侧设计建议**:

1. **关键交付物必须有确定性兜底**:将"不依赖模型自觉"的动作放在 tool 内部完成(如 preview 工具在返回时自动拼好 base 地址、自动落盘),而不是指望模型多调一次工具;
2. **降级安全**:强制类步骤(如生图)失败或被跳过时,交付流程仍应完整可用,并在交付说明中如实告知;
3. **提示词写法**:在 skill 提示词中把步骤写成"完成 XX 后**立即真实调用** tool:xxx,禁止只输出文字承诺",并给出不调用的后果(如"未配图将直接交付纯文字版并说明");
4. **不要把关键校验交给模型**:URL 拼接、文件命名、格式规范化等确定性转换一律在 tool 实现内完成。

> 平台侧 roadmap:对"必须调用"类步骤的编排保障(如 step-wise 执行校验)在评估中,当前版本请按上述模式设计。
